...
if [ "$BUILD_TYPE" = "testing" ]; then
  RELEASE_TAG="${SUSFS_VERSION}-testing-${DATE_TAG}"
  RELEASE_NAME="Sapphire Kernel ${SUSFS_VERSION} — ⚠️ Testing"
  IS_PRERELEASE="true"
else
  RELEASE_TAG="${SUSFS_VERSION}"
  RELEASE_NAME="Sapphire Kernel ${SUSFS_VERSION}"
  IS_PRERELEASE="false"
fi
